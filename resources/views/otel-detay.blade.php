...
<head>
    ...
    <title>{{ $otel->name['tr'] ?? 'Otel Detayı' }} — Dioreal Dijital</title>
...
<body class="{{ ($otel->theme_color ?? 'white') === 'black' ? 'theme-dark' : '' }}">
    ...
    <div class="page-hero" style="background-image: url('{{ asset($otel->img) }}');">
        <div class="page-hero-content">
            ...
            <h1 class="page-title lang-text-en">{{ $otel->name['en'] ?? '' }}</h1>
        </div>
    </div>
    ...
    <section class="detail-container">
        <div class="detail-grid">
            ...
            <div class="detail-story reveal">
                ...
                <div class="story-content-wrapper" style="display: flex; gap: 2.5rem; flex-direction: {{ ($otel->photo_layout ?? 'left') === 'left' ? 'row' : 'row-reverse' }}; align-items: flex-start;">
                    <div class="story-photo" style="flex: 1; min-width: 280px; max-width: 45%;">
                        <img src="{{ str_starts_with($otel->img, 'data:') || str_starts_with($otel->img, 'http') ? $otel->img : asset($otel->img) }}" alt="{{ $otel->name['tr'] ?? '' }}" style="width: 100%; border-radius: 12px; object-fit: cover; box-shadow: 0 10px 30px rgba(0,0,0,0.08);">
                    </div>
                    <div class="story-text" style="flex: 1.5;">
                        <div class="lang-text-tr">
                            {!! nl2br(e($otel->long_desc['tr'] ?? ($otel->desc['tr'] ?? ''))) !!}
                        </div>
                        <div class="lang-text-en">
                            {!! nl2br(e($otel->long_desc['en'] ?? ($otel->desc['en'] ?? ''))) !!}
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="detail-sidebar-card reveal" style="transition-delay: 0.2s">
                ...
                <a href="https://wa.me/{{ $settings['whatsapp'] ?? '905320000000' }}?text=Merhaba,%20{{ urlencode($otel->name['tr'] ?? $otel->name['en'] ?? 'Otel') }}%20hakkında%20detaylı%20bilgi%20ve%20rezervasyon%20talebinde%20bulunmak%20istiyorum." 
                   ...
                   class="btn-booking">
                    ...
                    <span data-i18n="detail_booking_hotel">Rezervasyon Yap</span>
                </a>
            </div>
    ...
    <section class="gallery-section reveal">
        ...
        <div class="gallery-grid">
            @if(!empty($otel->gallery) && is_array($otel->gallery))
                @foreach($otel->gallery as $g)
                    <div class="gallery-img-wrapper">
                        <img src="{{ str_starts_with($g, 'data:') || str_starts_with($g, 'http') ? $g : asset($g) }}" alt="Otel Görseli">
                    </div>
                @endforeach
            @else
